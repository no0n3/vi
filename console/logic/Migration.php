...
class Migration {
    ...
    public static function create($params) {
        ...
        $file = fopen(Vi::$app->params['sitePath'] . 'console/migrations/' . $migrationName . '.php', 'w');
...
CONTENT
    ...
    private static function applyMigrations($migrations) {
        if (!empty($migrations) && is_array($migrations)) {
            ...
            foreach ($migrations as $migration) {
                require Vi::$app->params['sitePath'] . "console/migrations/$migration.php";

                echo "Applying migration '$migration'...\n";
                $inst = new $migration();
                $inst->up();
                echo "INSERT INTO `" . self::MIGARTION_TABLE_NAME . "` (`name`, `time`) VALUES ('$migration', " . time() . ")\n";
                var_dump(Vi::$app->db->executeUpdate("INSERT INTO `" . self::MIGARTION_TABLE_NAME . "` (`name`, `time`) VALUES ('$migration', " . time() . ")"));
    ...
    private static function getPressentMigration() {
        ...
        $dh  = opendir(Vi::$app->params['sitePath'] . 'console/migrations');
    ...
    private static function getAplliedMigration() {
        ...
        if ($stmt = Vi::$app->db->executeQuery('SELECT `name` FROM `' . self::MIGARTION_TABLE_NAME . '`')) {
    ...
    private static function createMigrationTableIfNotExists() {
        if (!self::doesMigrationTableExist()) {
            ...
            return Vi::$app->db->executeUpdate(<<<QUERY
...
QUERY
    ...
    private static function getTables() {
        if ($stmt = Vi::$app->db->executeQuery('show tables')) {
